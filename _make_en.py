...
PAIRS = [
    ...
    ('<a class="skip" href="#works">К работам</a>', '<a class="skip" href="#works">Skip to work</a>'),
    ...
    ('<p class="flow__title">Мультиагентный конвейер</p>',
     '<p class="flow__title">Multi-agent pipeline</p>'),
    ('aria-label="Схема конвейера: источник, координатор, три агента, свод, очередь и оператор"',
     'aria-label="Pipeline diagram: source, coordinator, three agents, merge, queue and operator"'),
    ('<text x="150" y="31">Источник</text>', '<text x="150" y="31">Source</text>'),
    ('<text class="node__meta" x="150" y="45">форма, почта, API</text>',
     '<text class="node__meta" x="150" y="45">form, email, API</text>'),
    ('<text x="150" y="115">Координатор</text>', '<text x="150" y="115">Coordinator</text>'),
    ('<text class="node__meta" x="150" y="129">ставит задачи</text>',
     '<text class="node__meta" x="150" y="129">assigns work</text>'),
    ('<text x="36" y="206">Агент: разбор</text>', '<text x="36" y="206">Agent: parse</text>'),
    ('<text class="node__meta" x="36" y="220">достаёт данные</text>',
     '<text class="node__meta" x="36" y="220">extracts data</text>'),
    ('<text x="158" y="206">Агент: скоринг</text>', '<text x="158" y="206">Agent: scoring</text>'),
    ('<text class="node__meta" x="158" y="220">считает приоритет</text>',
     '<text class="node__meta" x="158" y="220">sets priority</text>'),
    ('<text x="280" y="206">Агент: поиск</text>', '<text x="280" y="206">Agent: search</text>'),
    ('<text class="node__meta" x="280" y="220">похожие случаи</text>',
     '<text class="node__meta" x="280" y="220">similar cases</text>'),
    ('<text x="150" y="306">Свод</text>', '<text x="150" y="306">Merge</text>'),
    ('<text class="node__meta" x="150" y="320">приоритет и причина</text>',
     '<text class="node__meta" x="150" y="320">priority and reason</text>'),
    ('<text x="150" y="392">Очередь</text>', '<text x="150" y="392">Queue</text>'),
    ('<text class="node__meta" x="150" y="406">порядок работы</text>',
     '<text class="node__meta" x="150" y="406">work order</text>'),
    ('<text x="150" y="476">Оператор</text>', '<text x="150" y="476">Operator</text>'),
    ('<text class="node__meta" x="150" y="490">решает по карточке</text>',
     '<text class="node__meta" x="150" y="490">decides on card</text>'),
    ...
    ('<p class="sec__lead">Пять направлений. Основное: агентные системы и LLM.</p>',
     '<p class="sec__lead">Five directions. The core one is agentic systems and LLM.</p>'),
    ...
    ('aria-pressed="false">ИИ и агенты</button>', 'aria-pressed="false">AI and agents</button>'),
]
...
CASE_PAIRS = [
    ...
    ('>RutaPay, платёжный шлюз в LatAm</a></li>', '>RutaPay, payment gateway for LatAm</a></li>'),
    ('>Northern Monkey, маркетинговое агентство</a></li>', '>Northern Monkey, marketing agency</a></li>'),
    ...
    ('<b>ТендерПульс</b>Воронка этапов с суммами', '<b>TenderPulse</b>Stage funnel with amounts'),
    ('<b>ТендерПульс</b>Заказы: фильтры и просрочка', '<b>TenderPulse</b>Orders: filters and overdue'),
    ...
    ('<b>Мотошкола «Трек»</b>Набор на обучение', '<b>Motorcycle school Trek</b>Enrolment for training'),
    ...
    ('<p class="step__out">На выходе: живая ссылка</p>', '<p class="step__out">You get: a live link</p>'),
    ...
    ('<li>Деплой на поддомене с сертификатом</li>', '<li>Deploy to a subdomain with a certificate</li>'),
    ...
    ('<li>Архитектура, API, интерфейс, база</li>', '<li>Architecture, API, interface, database</li>'),
    ('<li>Админ-панель, роли, права, отчётность</li>', '<li>Admin panel, roles, permissions, reporting</li>'),
    ('<li>Агентный слой внутри продукта</li>', '<li>The agent layer inside the product</li>'),
    ...
    ('<summary>Что если результат не понравится?</summary>', '<summary>What if I do not like the result?</summary>'),
    ...
    ('<summary>Как вы считаете стоимость?</summary>', '<summary>How do you price the work?</summary>'),
    ...
    ('<summary>Что с поддержкой после сдачи?</summary>', '<summary>What about support after handover?</summary>'),
    ...
    ('<summary>Что будет с моими данными?</summary>', '<summary>What happens to my data?</summary>'),
    ...
    ('<summary>Можно начать с маленькой задачи?</summary>', '<summary>Can we start with something small?</summary>'),
    ...
    ('<summary>Как быстро начнёте?</summary>', '<summary>How soon can you start?</summary>'),
    ...
    ('<label for="b-contact">Как с вами связаться</label>', '<label for="b-contact">How to reach you</label>'),
    ('placeholder="Telegram, почта или телефон"', 'placeholder="Telegram, email or phone"'),
    ('<label for="b-task">Что нужно сделать</label>', '<label for="b-task">What needs to be done</label>'),
    ...
    ('<label for="b-pain">Что сейчас не работает</label>', '<label for="b-pain">What is not working now</label>'),
    ...
    ('>Собрать заявку и открыть Telegram</button>', '>Build the request and open Telegram</button>'),
    ...
    ('<p class="brief__next-title">Заявка готова</p>', '<p class="brief__next-title">Request is ready</p>'),
    ...
    ('<li>Оценку по объёму и дату сдачи</li>', '<li>An estimate by scope and a delivery date</li>'),
    ...
    ('<span>Удалённо, часовой пояс GMT+12</span>', '<span>Remote, GMT+12 time zone</span>'),
...
PAIRS += [
    ...
    ('<span class="case__label">Зачем заказчику</span>', '<span class="case__label">Why it matters</span>'),
    ('<span class="case__title">Лендинги</span>', '<span class="case__title">Landing pages</span>'),
    ('<span class="case__title">ТендерПульс</span>', '<span class="case__title">TenderPulse</span>'),
